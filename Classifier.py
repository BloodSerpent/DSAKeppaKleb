...
class Classifier:
    ...
    def fit(self,batch_size,epoch):
        ...
        model.fit_generator(datagen.flow(self.x_train, self.y_train, batch_size=batch_size),steps_per_epoch=len(self.x_train) / batch_size, epochs=epoch)
...
datagen.fit(image)
# fits the model on batches with real-time data augmentation:
model.fit_generator(datagen.flow(image, label, batch_size=16),steps_per_epoch=len(image) / 16, epochs=200)
